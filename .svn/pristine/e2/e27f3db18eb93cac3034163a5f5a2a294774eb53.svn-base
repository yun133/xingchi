						...
						<div class="portlet box purple">
							...
							<div class="portlet-body">
								...
								<div class="clearfix">
									...
									<div class="btn-group">
									<a href="<?php echo U('Type/type_add');?>">
										<button class="btn green">
										添加类型 <i class="icon-plus"></i></button></a>
...
<table class="table table-striped table-hover table-bordered" id="sample_editable_1">
									...
									<?php if(is_array($data)): foreach($data as $key=>$v): ?><tr class="">
											...
											<td><a class="" href="<?php echo U('Attribute/attr_list',array('type_id'=>$v['id']));?>">属性管理</a></td>
											<td><a class="" href="<?php echo U('type_edit',array('id'=>$v['id']));?>">Edit</a></td>											
	...
	c_url="<?php echo U('Type/type_del','',false);?>"
		// function deltype(id) {
		// 	if (confirm('您确定要删除这个类型吗?')) {

		// 		$.getJSON("<?php echo U('Type/type_del');?>",{"id":id},function(data){
		// 			if (data.status==1) {
		// 				location.reload();
		// 			}else{
		// 				alert(data.status);
